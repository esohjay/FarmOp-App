...
router.get(
  ...
  catchAsync(async (req, res) => {
    ...
    let breed = [];
    let pStage = [];
    const farmstock = await FarmStock.paginate(
      dbQuery || { creator: req.user.farmId },
      options
    );
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    const exist = await FarmStock.find({
      $and: [{ creator: req.user._id }, { tag: farmstock.tag }],
    });
    if (exist.length) {
...
router.put(
  ...
  catchAsync(async (req, res) => {
    ...
    const exist = await FarmStock.find({
      $and: [{ creator: req.user._id }, { tag: tag }],
    });
    if (exist.length) {
      req.flash("error", "Tag Number already exist");
      error = "Tag Number already exist";
